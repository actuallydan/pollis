...
- **The client holds NO database credential (#987).** Every remote read and every remote write is a typed `POST /v1/...` on `pollis-delivery` (api.pollis.com / api-dev), called via `pollis-core/src/commands/mls/ds_client.rs`: `ds_post*` = device-signed ML-DSA-44 `X-Pollis-*` headers; `ds_post_session*` = OTP-session bearer for bootstrap/pre-enrollment writes; `ds_post_plain` = the pre-credential endpoints (OTP, account-probe, enrollment poll). Reads live in `pollis-core/src/commands/ds_reads.rs` (+ `mls/ds_reads.rs` for the control plane); wire types in `pollis-api`. Never add a client-side remote SELECT/INSERT/UPDATE/DELETE — `pollis-core` does not depend on `libsql` at all, so there is nothing to add one with. Extend a DS endpoint instead.
- **Storage:** Turso = public metadata + encrypted envelopes (never plaintext or private keys). Local SQLite (`state.local_db`) = message ciphertext + MLS state (never users/groups/channels tables — those are remote-only). Device keystore = identity key pair + session token; backend chosen at runtime (`keystore.rs`) — OS keychain where one exists, machine-bound encrypted file where none does, plaintext never (#882).
...
- **Workspace:** `pollis-core` (backend; mobile via uniffi), `src-tauri` (shipping shell — Electron is history), `pollis-delivery` (DS), `pollis-schema` (the remote migrations, shared so the DS needn't hand-roll test tables), `pollis-sqlcipher` (the SQLCipher the Windows build links — RustCrypto crypto provider, no OpenSSL), `pollis-tui` (terminal client), `pollis-capture-{linux,macos,proto}` (camera/screen helper subprocesses), `verifiable-log*` (transparency log), `frontend/`, `website/` (static HTML, Cloudflare Pages).
...
- **Security model:** trusted = device, local DB, signed app binary, device keystore (OS keychain, or — where none exists — an encrypted file bound to the machine, which stops a *copied* keystore but not a same-UID local attacker or a full-disk image; see `docs/security-whitepaper.md` §3.5). Untrusted = network, Turso, DS, server operators (they see metadata, never content or keys).
...
- **Messages must work; history is bounded, not flaky.** Exactly three acceptable losses: (1) messages sent before you joined the MLS tree, (2) a new device starts empty (no key backup — never add Megolm-style backup unless explicitly asked), (3) *#720* a device silent past `POLLIS_DS_WATERMARK_STALE_MONTHS` (12) stops pinning envelope retention, so mail waiting only on it may be collected — keyed on the device's last report, never on message age, and reversible the moment it reports again. (1) and (2) are cryptographic; (3) is a deliberate policy bound against unbounded storage growth, and adding a fourth needs the same scrutiny. Everything else — delivery to every current member, decryption, offline/online cycles, reconnects, DM-request accepts — must work. Given "simpler but drops messages" vs "more complex but delivers," pick the one that delivers.
...
- **Never produce unsolicited wrap-up deliverables** — no artifact pages, PDFs, or polished "portfolio" reports summarizing work just done. End with a concise terminal summary. Produce such a document only when explicitly asked. Absolute.
...
- No neon/glow effects — solid borders and backgrounds only. No periodic polling (`setInterval` keepalives) — event-driven, or lazy recovery on the next attempt. (#987 removed `RemoteDb::with_retry`, the one narrow exception, along with the client's database handle; there is no sanctioned retry helper any more.)
...
- **Tailwind-first, token-backed:** design tokens are CSS variables in `frontend/src/index.css`, surfaced as semantic utilities in `frontend/tailwind.config.js` (`bg-bg`, `bg-surface[-raised|-high]`, `text-fg/dim/muted/accent`, `border-line[-strong]`, `hover:bg-hover`, `h-bar`). Use the utilities — no `[var(--c-…)]` arbitrary classes or inline token styles; if a utility is missing, add it to the theme. Inline `style` only for runtime-computed values; co-located CSS files (or `@layer components`) for pseudo-elements, keyframes, and complex selectors.
