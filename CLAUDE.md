...
- **Remote reads are direct to Turso** (`state.remote_db`, SELECT only). **Remote writes go through the DS only** — typed `POST /v1/...` on `pollis-delivery` (api.pollis.com / api-dev), called via `pollis-core/src/commands/mls/ds_client.rs`: `ds_post*` = device-signed ML-DSA-44 `X-Pollis-*` headers; `ds_post_session*` = OTP-session bearer for bootstrap/pre-enrollment writes; `ds_post_plain` = the OTP endpoints themselves. Never add a client-side remote INSERT/UPDATE/DELETE — extend a DS endpoint.
...
- No neon/glow effects — solid borders and backgrounds only. No periodic polling (`setInterval` keepalives) — event-driven, or lazy recovery on the next attempt. `RemoteDb::with_retry` is the narrow version of that for remote reads (one reconnect, one retry; see its doc for which call sites qualify) — it is not a general-purpose retry loop, so do not cite it as cover for one.
