...
- `created_at` TEXT NOT NULL DEFAULT now
- `uploaded_by` TEXT _(migration `000031`; **dead since migration `000032`** — never written, never read, and existing values cleared. Not dropped because migrations here are additive only)_
...
**The derived reference count is the whole rule, for everybody.** `object_is_referenced` takes a `content_hash` and nothing else — no actor, no uploader — and BOTH lifetime decisions read that one predicate: `apply_delete_attachment` for the Turso row and `/v1/r2/presign` for the R2 blob (`broker.rs`), so the row and the object cannot disagree about whether those bytes may go. Messages belong to users, so "my references" are exactly the declarations naming my messages: you release yours by deleting your messages, and the object survives past that only while somebody else's live message still carries the same file.

**The uploader exemption (migration `000031`) was a mistake, and is gone.** It recorded `attachment_object.uploaded_by` / `attachment_ref.registered_by` and gave the first uploader a delete that excluded other accounts' references — so account A deleting its own file destroyed the blob account B's live message depended on, 404'ing B's attachment in a conversation A cannot see. That is not a security property and was never a trade worth making: one blob backing every copy is a *storage optimisation*, and an optimisation must not be visible to users as "someone else deleted my file". Duplicate blobs would have been better. Both columns are now dead (migration `000032` stops writing them, clears the values, and nothing reads them); the index `idx_attachment_ref_registrar` is dead with them. A later release may drop all three once no deployed DS mentions them.

What the #1161 M5 review actually found and what stays fixed: a reference could be **forged** — you could declare `(victim_hash, message_id)` naming a message you do not own and pin their blob. #1165 fixed that by binding every declaration to `message_envelope.sender_id` (`apply_register_attachment` refuses otherwise), and that binding stays. The review's second half — "the uploader's delete presign 403s while others reference the blob" — was mis-framed as a defect. Under plain reference counting it is correct behaviour: you do not get to destroy bytes another user's live message still depends on. An honest re-send and an abusive "pin" are the same act precisely because neither denies the uploader anything real — their own reference goes when their message goes, their own access goes with it, and the bytes persist only because somebody else legitimately still has that file.
...
- `created_at` TEXT NOT NULL DEFAULT now
- `registered_by` TEXT _(migration `000031`; **dead since migration `000032`** — never written, never read, and existing values cleared. The #1165 binding is enforced at registration, so no later reader needs to know who declared a row. Not dropped because migrations here are additive only; the index `idx_attachment_ref_registrar` over it is dead too)_
...
**A declaration is bound to its message's author (#1161 M5, fixed in #1165).** `apply_register_attachment` refuses a `(content_hash, message_id)` unless `message_id` names a still-existing envelope whose `sender_id` is the verified caller, so you cannot declare a reference for somebody else's message and pin their blob. The binding is enforced at write time and nothing downstream re-checks it — every stored row joined to a live envelope was written by that envelope's own author, which is why the liveness predicate needs no identity of its own.
...
- **cert publish** (`pollis-delivery` `bootstrap.rs::seed_conversation_watermarks`, called from `apply_publish_device_cert`) seeds per conversation the user is already a member of, for the newly enrolled device. It used to run at `register_device`, one step earlier — but a device that has only registered has proved nothing beyond holding an OTP session: a sibling still has to approve it, and it may never publish a cert at all. Seeding there let such a never-enrolled ghost pin every conversation's envelopes for the whole #720 staleness window (#1092). Cert publish is the pivot where the device becomes real, so that is where it starts pinning.
...
**Rollout, and why it took one step rather than two (#1157).** #1122 landed the handle alongside `conversationId` on the usual additive plan — send both, prefer the handle, drop the old field once clients that prefer it are the floor. #1157 then dropped it immediately, because the floor turned out to be vacuous: **mobile has never been distributed** (#712, #340 and #341 are all still open), and mobile is the only push consumer — desktop never registers a token. There is no shipped client reading `conversationId`, so waiting a release would have bought nothing and left the disclosure open for it.

The payload is now `data: { h }` and nothing else, and `mobile/lib/push/routing.ts` has no `conversationId` branch at all — a payload carrying one is *ignored*, so a rolled-back DS or a malicious push to a stolen token cannot re-enable the disclosure by putting the field back.
...
Retired by #1085. Migration `000025` emptied them (the frozen snapshot was real membership metadata about real users, and once the DS stopped purging on teardown it would otherwise have lingered), and the DS no longer names them anywhere — the eight DELETE sites in `teardown.rs`, `groups.rs` and `profile.rs` are gone. `pollis-delivery/tests/retired_directory_mirror.rs` fails if any SQL in the crate starts naming them again, keyed on SQL keyword adjacency so prose about the decision does not trip it.

**Dropped by migration `000029` (#1143).** #1085 deliberately left the tables in place, because a rolling deploy still had older DS instances executing the removed DELETEs and pulling the tables out from under them would have failed account and group deletion. Several DS deploys later that no longer holds, so `000029` drops both. That is the second half of the multi-release dance a `DROP` requires: release N stopped naming them, release N+1 removes them — and because no client has ever read or written these tables, no client release floor applied. `the_retired_tables_are_gone_from_the_schema` asserts the DROP actually took effect (with a positive control, so it cannot pass vacuously against an unapplied schema), and the two `EXEMPT_FROM_*_PURGE` entries are gone with the tables.
