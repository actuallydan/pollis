...
- **`watermark::delete_consumption_tests`** (`pollis-core`, #693) pins the asymmetry candidate 2 turns on: `next_watermark` consumes a `delete` tombstone UNCONDITIONALLY (`is_handled(Delete) == true`), advancing the cursor past it on the first pass, whereas an undecryptable `message` is held back and re-fetched to success. So in `ingest.rs`, where the tombstone redaction is `let _ = …execute("UPDATE message SET content = NULL …")`, a redaction that fails to land (row absent, or a transient local-DB error) is consumed-and-lost — never retried. The cursor is not the culprit (proven), so a dropped delete is an *application* failure.
