...
- **`tests/envelope_query_plan.rs`** (`pollis-delivery`, #1049) is the repo's first **cost** test, and it exists because cost is invisible to every other kind. `fetch_envelopes` was correct before and after the regression it guards: #875 batched N per-channel fetches into one and turned a bound `?1` watermark into a subquery correlated on `message_envelope.conversation_id`. Correlating strips the planner's ability to bound `sent_at`, so "is there anything new?" began reading every retained envelope in the conversation and discarding it — 36 ms and ~200k billed Turso rows for a caught-up device returning nothing, growing with history, on the path every client takes on every channel open and every realtime hint. **Rows returned were identical and every test passed.** The assertion is therefore on `EXPLAIN QUERY PLAN`, the only artefact that separates the two forms: the fixed query must contain `sent_at>?` and must not contain `CORRELATED`. A second test keeps the *old* query as a negative control — if SQLite ever learns to bound `sent_at` through a correlation, the first test has stopped discriminating and must be rewritten rather than passing for the wrong reason. Both run against the real schema via `pollis_schema::apply::single_db`, because the plan is a property of the indexes and a fixture missing one would prove nothing.
...
- **`messages::admin_delete_visibility_tests`** (`pollis-delivery`, #693/#661 WS1) isolates the flaky `sealed_admin_delete_of_other_member_works` and RULES OUT candidate 1 (a read-after-write visibility gap between the DS write and the recipient read). Driving the real DS `apply_send_message` / `apply_advance_watermark` / `apply_delete_message` across TWO connections of one shared libsql `Database` (the faithful model of the flows harness, whose clients read through the in-process DS over one shared handle), it proves the admin tombstone is ALWAYS written and ALWAYS returned by the recipient's next `seq > last_seq` fetch. Since #1087 it keeps the adversarial `sent_at` shapes — a whole-second stamp, a clock-ahead stamp, and a message stamped years BELOW the tombstone — and asserts delivery anyway: ordering is a sequence now, so those stamps are decoration and the test says so by construction. Its fetch helper calls the PRODUCTION query (`directory::envelopes_for_device`) rather than a copy of the predicate, which is what let it silently stop testing delivery when the cursor changed. `read_your_writes_holds_across_connections_of_one_shared_database` pins the underlying primitive. Conclusion: #661's residual flake is NOT delivery — it lives in the client's *application* of the fetched tombstone (candidate 2). The flows test itself now carries a read-only `diagnose_admin_delete_failure` that, on a live failure, classifies it as tombstone-never-written / never-fetched / fetched-but-not-applied.
- **`tests/cursor_poisoning.rs`** + **`messages::cursor_stamp_tests`** (`pollis-delivery`) pin the stamp bound that closed the blackout attack: a client-chosen `sent_at` (send AND edit) or `last_fetched_at` is admitted only as a canonical UTC RFC 3339 stamp within the signature window of the DS clock (`check_cursor_stamp`), and `/v1/watermarks/advance` is refused for a non-member and for a `device_id` other than the signing device. They fail against the verbatim-store code, where one `9999-…` stamp blacked out a conversation for every member and the next GC sweep collected it. **Since #1087 the stamp is no longer the cursor**, so these now guard a display value and the membership gate rather than delivery itself; the `TOMBSTONE_FLOOR` half is gone with the floor.
- **`messages::delivery_sequence_tests`** + **`delivery_sequence_backfill_tests`** (`pollis-delivery`, #1087) pin the cursor that replaced it. Sequences increase, are scoped per conversation, and cover every envelope kind; the load-bearing one is `a_sequence_is_never_reused_after_gc_empties_the_conversation`, which fails against the naive `MAX(seq)+1` — GC deletes rows, so a counter derived from them resets and hands out a position every device has already passed. The backfill tests run the REAL `000028` SQL against a pre-migration database and assert it preserves `sent_at` order, seeds the counter above what it assigned, and maps each existing cursor so nothing is re-delivered or skipped.
