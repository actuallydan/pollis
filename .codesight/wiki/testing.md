...
- **`tests/cursor_poisoning.rs`** + **`messages::cursor_stamp_tests`** (`pollis-delivery`) pin the delivery-cursor bound: a client-chosen `sent_at` (send AND edit) or `last_fetched_at` is admitted only as a canonical UTC RFC 3339 stamp within the signature window of the DS clock (`check_cursor_stamp`), `/v1/watermarks/advance` is refused for a non-member and for a `device_id` other than the signing device, and `TOMBSTONE_FLOOR` reads only member-device rows (`messages::tombstone_floor_tests::watermark_rows_outside_the_member_device_roster_do_not_floor_the_tombstone`, plus the DM-roster shape). Each fails against the verbatim-store / unjoined-floor code, where one `9999-…` stamp blacked out a conversation for every member and the next GC sweep collected it. `admin_delete_visibility_tests` now fixes its "client clock ahead" base second inside that window (`base_second_ahead`) rather than in 2027, so it still exercises `sent_at_after`'s floor branch through the real, bounded send path.
