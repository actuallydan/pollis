...
4. `cargo test -p pollis-core` — MLS unit tests, and the local-database at-rest tests (#992): `the_local_database_file_is_encrypted_at_rest` writes a message through `LocalDb::open_for_user` and then reads the file's raw bytes back, and `the_crypto_provider_is_the_one_this_platform_documents` (#NNN) asks `PRAGMA cipher_provider` whether the build got the provider `docs/security-whitepaper.md` §7.0 documents for this platform.
...
The workflow has a second heavy job, `macos-at-rest` (#NNN), gated on the same
...
as `windows-link.yml` does. It exists because until #NNN **no macOS runner in
...
`the_crypto_provider_is_the_one_this_platform_documents` (#NNN). The step greps
