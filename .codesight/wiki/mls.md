...
6. **Diff**: desired set vs actual tree → compute adds and removes. The desired set (`desired_set`) is the union of two sources — devices with an available KeyPackage (the adds) and existing leaves whose user is still on the roster (the retentions, which is what stops the committer evicting itself). **Both** are gated on `valid_devices`, the `revoked_at IS NULL` snapshot from `registered_devices`. Gating only the retention half was #679: a revoked device rode back in through its own leftover KeyPackage, so it never reached `to_remove` and its leaf survived. `valid_devices = None` (snapshot unreadable) disables the gate; `Some(empty)` means nothing is valid — the two are deliberately different states, so a transient `user_device` read failure cannot empty a group. A `Some(empty)`/short answer is still indistinguishable from "everyone was revoked", so #1082 adds a **self-presence gate** (`reconcile::snapshot_includes_actor`) ahead of the diff: the actor runs the pass as a roster member, so its own `(user_id, device_id)` must appear in the snapshot, and if it does not the whole pass is refused (and `migrate` defers) rather than evicting every other leaf on one bad read. An empty roster (legitimate teardown) and an unknown own `device_id` (cannot evaluate) deliberately do not gate. A third gate uses the identity directory: a leaf already in the tree whose signature key is **positively** not its user's certified key (`LeafVerdict::Uncertified` — revoked, cert invalid, or a key that is not the certified one) is evicted like a revoked device, and left out of `actual` so the device's genuine KeyPackage (if any) can be added in the same commit. `Unverifiable` leaves are retained: absence of a row is not evidence. Our own leaf is never judged. This is how a rogue leaf grafted in by a malicious or pre-fix committer leaves the tree — append-only, on the next honest member's reconcile.
7. **Build and stage commit** with both add and remove proposals — do NOT `merge_pending_commit` yet
8. **Submit the commit bundle to the DS on a fresh connection**: commit + GroupInfo + Welcome(s) are one **atomic** `POST /v1/commits` — the DS writes all three in a single libsql transaction (see "MLS durability hardening" below), so a recipient never sees a commit with no matching Welcome. The body also names **every** added user (`added_user_id`, CSV) and device (`added_device_ids`, CSV). These are a *prefetch hint* — the DS folds the named users' cert rows into the commit batch replaying members fetch (`reads::added_identities`, which splits both columns and queries per user) — not what they verify, which is the commit itself (below).

   **Every** is load-bearing, and both writers now honour it (#1080). One commit routinely adds devices of two users: `desired_set` admits every roster device with a claimable KeyPackage, so an enrolment that coincides with an invite, an approval or a DM add lands in one commit, and a suite migration re-adds the *whole roster* at once. Naming only the first user left every other member's leaf `Unverifiable` to a replaying device — which is not a security hole (verification reads the commit, not the columns) but did cost a pointless eviction reconcile on every receiver and a log line saying a teammate's device could not be verified. `reconcile` was fixed first; `migrate::added_metadata` kept the single-user projection until #1080, and only an external join into a fresh lineage could see it, because a Welcome joiner starts at the successor's epoch 1 and never replays its opening commit
...
3. Other devices call `process_pending_commits_inner` which:
   ...
   - Applies them sequentially via `apply_one_commit` — which first requires the row to be a `ContentType::Commit` for *this* `GroupId` before openmls ever processes it (#1161 C2/H5: the DS never parses commit bytes, so a member could otherwise post an application message that either advanced every replayer's epoch counter past the real group epoch — permanently losing delivery for everyone — or, read back as `OwnPrivateMessage`, merged a staged commit onto a branch the log never accepted). It returns the leaves each commit added, read off the staged commit's own Add proposals **plus a before/after tree diff (`grafted_leaves`), so a leaf arriving in an external-join UpdatePath is verdicted like an Add rather than skipping verification entirely (#1161 H4)** (`AddedLeaf`: credential `user:device`, leaf signature key, scheme). Every added leaf is checked against the batch's `IdentityDirectory` (built from the snapshot's `added_identities`); a leaf that is not `Certified` is logged and answered with a detached eviction reconcile (`report_uncertified_leaves`). **Only a POSITIVELY `Uncertified` leaf also writes an `uncertified_mls_leaf` `security_event`** to this user's own audit log (the Security page lists it). An `Unverifiable` one does not (#1127): "the cert row has not reached me yet" — or "the commit's `added_user_id` hint did not name this user" — is the absence of evidence, the reconcile self-corrects it, and alarming on it would both train people to ignore the real event and let a malicious committer manufacture alarms in other members' audit logs by omitting the hint. The split is `FlagKind` / `FlagKind::alarms`. The commit is still merged: it won the epoch CAS and is canonical, and a `StagedCommit` cannot be dropped and re-processed later (its ratchet generation is consumed), so the remedy is eviction, not refusal. Because the leaves come from the commit and not from the `added_*` columns, a committer that NULLs or shortens the hint can only make its own honest add look *unverifiable* (which the eviction reconcile, reading the whole roster's material, then finds certified and keeps) — never make a rogue add look verified. See [Leaf cross-signing](#leaf-cross-signing)
   ...
   - **Adopting a lineage is gated twice (2026-09-18 security review, #1161 M6).** A `GroupId` is not a capability — anyone can build an MLS group with an arbitrary one — so a hostile DS could otherwise stand up its own group named `{conversation_id}#g{N+1}`, answer `head_generation = N+1`, and have every device external-join it and delete the predecessor's keys. First gate: `build_external_commit` refuses a GroupInfo whose `GroupId` is not `mls_group_id(conversation_id, generation)`, which pins the lineage's *name*. Second gate: `check_lineage_provenance` pins its *provenance* — it verifies the ratchet tree out of the GroupInfo, recovers the signing leaf by trial verification against each leaf's key (the pinned openmls rev keeps `signer()` `pub(crate)`, and checking the signature under a leaf's key is the same operation an index would have needed anyway), and verdicts that leaf against the pinned `IdentityDirectory`. A DS holds no account identity key, so it cannot produce a certified signer.
     The decision is the pure `invariants::lineage_adoption`, and its **bias is deliberate**: it refuses only on a positive `Uncertified` or a signer demonstrably off the roster. `Unverifiable`, and a read that simply failed, both **adopt** — a device recovering with no local state must not be locked out of recovery by a cert row that has not propagated yet. A 3×3 truth-table test exists so that widening refusal into the `Unverifiable` column fails there rather than in production.
     **Not covered:** a lineage founded by *Welcome* rather than external commit. `join_from_welcome` cannot read the conversation id until the KeyPackage has opened the blob, so the identity read cannot precede the join; that path is guarded instead by the epoch guard, the filed-under check and `welcome_is_corroborated` (#1161 C1). Predecessor chaining was considered and rejected: the chain is only verifiable by a device that still holds predecessor state, which is exactly what this gate's callers do not have.
...
### One group load per epoch, not per message (#875)

The decrypt path is `MlsDecryptor` (`mls/group_state.rs`): a group loaded ONCE
for the run of envelopes sealed at the epoch it sits at. `try_mls_decrypt` is
that type used for a run of length one, and until #875 it was the whole path —
every single envelope re-read the join config, group context, ratchet tree,
transcript hash, confirmation tag, leaf index, group state, message secrets,
resumption PSKs and proposal queue out of `mls_kv`, re-parsed the lot, decrypted
one message and threw it away. **Measured: 12 `mls_kv` SELECTs + 1 UPDATE per
message, now 12 SELECTs for the whole run + 1 UPDATE per message** — a
200-message catch-up went from 2400 statements to ~212.

Holding the group across a run is not a shortcut. Decrypting an application
message mutates exactly one thing — the secret tree inside
`message_secrets_store`, which ratchets forward so a consumed key cannot be
reused — and openmls persists that itself, inside `process_message`, before it
returns (`will_modify_secret_tree` → `write_message_secrets`), precisely so
forward secrecy survives a crash mid-batch. Nothing else about the group changes
during application-message processing. So the reload this replaced was reading
back the bytes the previous decrypt had just written, and a crash half-way
through a run leaves exactly the state a crash half-way through the old loop
left. That per-message write is forward secrecy and must never be optimised
away; `a_run_of_envelopes_loads_the_group_once` asserts it is still there.

**A decryptor must never outlive its epoch.** A commit changes the group out from
under a held copy. `ingest_group_envelopes_interleaved` opens one per `on_epoch`
...
than anecdotal. Its three statements also go through `prepare_cached` now — there
are exactly three and they run a dozen-plus times per message, so the SQL was
being re-parsed for query plans that never change.
