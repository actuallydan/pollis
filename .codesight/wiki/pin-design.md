...
- **#194** (merged) — PIN is now load-bearing. `db_key` and `account_id_key` exist on disk only as ciphertext in `*_wrapped_{uid}` slots. The local SQLCipher DB cannot open until `AppState.unlock` is populated, which only happens after `set_pin` (initial), `unlock` (correct PIN), or a fresh signup / enrollment unwrap that hands raw bytes directly to `unlock` without ever writing them to the keystore.
- **#951** (merged) — the cutover off the unwrapped legacy slots is finished. The wrap-then-delete sequence in `set_pin` now runs through `pin::install_wrapped_slots`, which **reads the three new slots back and unwraps them** before deleting `db_key_{uid}` / `account_id_key_{uid}`. Until that read-back succeeds the legacy slots are the only copy of an unrecoverable key on the device, and a `store` that returns `Ok(())` is a claim rather than evidence — a keychain can accept a write it will not return. Any failure leaves the legacy slots untouched and asks the user to retry. Pinned by `legacy_slots_are_wrapped_then_removed`, `an_interrupted_migration_loses_nothing` (covers both the loud and the *silent* write failure) and `a_user_with_no_legacy_slot_is_unaffected` in `commands/pin.rs`.
...
- `migrate_set_initial_pin` was never built. The migration path it describes (§ "Migration for existing installs") did not ship as a distinct command — existing installs go through the ordinary `set_pin` flow. What DID ship, in `install_wrapped_slots`, is that section's step (e): the unwrapped slots are deleted only after the wrapped ones are written *and proven readable* (#951). The `session_*` blob is not the commit marker the note proposed; the successful read-back is.
...
- The enrollment verification code is not user-chosen or separately generated: it is **derived** from the requesting device's ephemeral public key via HKDF (`derive_verification_code`, 8 characters over a 32-symbol alphabet), so a swapped key yields a different code. The HKDF is **salted with a length-prefixed `request_id ‖ user_id ‖ created_at` transcript**, so the code is a function of *this request* rather than of the key alone — without that salt a 2^40 key→code table would have been computable once and reusable against every user forever, which is what the 2026-09-18 review found. See #793, #1096 and `ARCHITECTURE.md` § Multi-device enrollment.
