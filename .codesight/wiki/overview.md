...
- **Deployable node** (`pollis-relay` bin): TOML config file (bind / allowlist / persisted QUIC identity / rate limits / `allow_extend` / `directory_url` + `directory_key_b64` / `extend_dial_timeout_secs`), generate-and-persist self-signed QUIC identity, graceful shutdown (drain on SIGTERM/SIGINT), and in-memory per-account / per-IP rate + concurrency limits (`Rejected(RateLimited)`). The **unauthenticated surface** is bounded separately: every connection is QUIC-Retry address-validated, the global connection cap counts completed handshakes only (half-open ones have their own bound), connections are capped per source IP (`max_connections_per_ip`, default 32), a stream must finish negotiating within `first_frame_timeout_secs` (10) or is refused and its circuit slot freed, and a connection may hold at most 4 bi-streams. Per-IP limits key on the QUIC peer address for **every** stream — including one that opens with `Layer`, which is unauthenticated and would otherwise be a one-frame bypass (there is no account-only admission path in `ratelimit.rs`; the volunteer peer engine keeps the per-IP limits **finite but non-discriminating** — everything reaches it over the loopback bridge, so they are sized above what its global connection cap can produce rather than lifted to `u32::MAX`, which is not a bound at all). The egress allowlist matches **`host:port`**: a bare entry means 443 only, `host:*` opts a host into every port, and a malformed entry aborts startup (`pollis-relay/tests/limits.rs`). Stateless, disposable, rotatable; holds no Turso/DS credentials. `GET /version` reports the node's **build** identity (`sha`) and **protocol** identity (`protocol` = the newest ALPN it speaks, `pollis-relay/4`); the automated AWS pool's reconciler uses protocol identity for signed-directory membership (a wrong-generation node is excluded so no client can fail ALPN against it) and build identity to cycle nodes off a stale image, so an image roll converges the fleet without a mutable `:latest` tag (#703). See `docs/relay-operations.md` and `infra/relay-hydra/README.md`. **The AWS pool itself is currently SWITCHED OFF** (`hydra_enabled = false` in `infra/relay-hydra/variables.tf`): no nodes, no ASGs, no reconciler, no schedule — the directory hosting and SSM params are kept so one `terraform apply` brings it back. Everything described here is how the pool behaves when on; with it off, `Off`/`Prefer` clients dial direct and only `Strict` degrades.
- **Multi-hop onion (wire v4, #813 Phase A, design §6.2).** A `Circuit` is an ordered `Vec<Hop>`; `n = 1` is the shipped v0 path, `n > 1` telescopes. Against each hop the client sends its device handshake plus `Extend(next)`; that relay QUIC-dials the next one (pinning the 32-byte cert fingerprint the frame carries), announces itself with a `Layer` frame, and splices — after which the client runs **one nested TLS 1.3 session per hop** (`pollis_relay::onion`, pinning that hop's leaf cert) straight through it. So each relay peels exactly one layer: hop 1 sees the client's address and hop 2's, a middle hop sees only its neighbours, and the **last hop** — always first-party — sees the destination and its predecessor, never the client. The destination allowlist is enforced wherever the `Connect` lands, which is by construction the last hop, so the closed-overlay guarantee holds at every path length. **`Extend` is bounded too** (`pollis-relay/src/nexthop.rs`): the frame carries a client-chosen `SocketAddr`, and the pinned fingerprint bounds who may *answer*, not who gets *dialled* — so a node dials a next hop only if the currently verified **signed directory** lists that exact `host:port` (its `peers[].parked_at` included), the address is public unicast (never loopback/RFC1918/CGNAT/link-local/unique-local/multicast/broadcast), and it is not this node's own entry; the leg is opened under a 5s deadline. Fail-closed like revocation — no pinned key, no directory yet, or an expired one and the node extends to nothing — and a parked peer is exempt because nothing is dialled (the fingerprint selects a connection that peer already opened). The volunteer peer engine gets the same store from `net::peer::reachability`, which already verifies that directory to decide where to park, and turns `allow_extend` **off** outright when it has none, so a consenting device is never an open dialer inside its owner's network. Versions negotiate purely in ALPN (a v4 relay still offers `pollis-relay/3`, so v4 clients and the shipped v3 fleet interoperate single-hop); `Extend`/`Layer` are refused below v4, so a v3 stream can't request forwarding. **What this does NOT hide:** every hop still verifies the device cert, so each learns *which account* built the circuit — the property delivered is network-address unlinkability, not anonymity.
