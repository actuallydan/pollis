...
// The keys everything on this page trusts. A *set*, not one key, so a signing-key
// rotation is not a flag day: during a changeover both the outgoing key (with the
// end of its overlap window) and the incoming one are listed, and either verifies.
// Mirrors PINNED_LOG_PUBLIC_KEYS in pollis-core/src/commands/transparency.rs and
// MUST stay byte-identical to it — the two are the same claim made to two
// audiences, and a disagreement is indistinguishable to a visitor from a
// compromised log. Change them in the same commit, always.
//
// `notAfter` is epoch milliseconds; null means the current key. Past that instant
// a retired key stops being accepted here, exactly as it does in the app.
// An empty set reports "cannot check" rather than raising an alarm: an absent pin
// must WITHHOLD trust, never manufacture a false one.
const PINNED_KEYS = [
  {
...
// The pinned keys still inside their overlap window, newest first.
function livePinnedKeys() {
  const now = Date.now();
  return PINNED_KEYS.filter(function (k) {
    return k.notAfter === null || now <= k.notAfter;
  });
}
...
function renderKeyPending(served) {
  byId("art-key-hex").innerHTML =
    ...
    "this page pins no unexpired key — nothing is being checked" +
    "</span>";
}

function renderKey(served) {
  const live = livePinnedKeys();
  if (live.length === 0) {
    renderKeyPending(served);
    return;
  }
  // A match against ANY unexpired pin is a match — that is what makes a rotation
  // invisible here. Only a key matching none of them is the hostile-host case.
  const hit = live.filter(function (k) {
    return k.publicKey.toLowerCase() === served.toLowerCase();
  })[0];
  const shown = hit || live[0];
  byId("art-key-hex").innerHTML =
    copyChip(shown.publicKey, shown.publicKey) +
    '<span class="art-badge ' +
    (hit ? "art-badge--pass" : "art-badge--fail") +
    '">' +
    (hit
      ? live.length > 1
        ? "✓ served key matches a pinned key (" + live.length + " valid during rotation)"
        : "✓ served key matches"
      : "✗ served key DIFFERS") +
    "</span>";
}
...
function renderKeyUnavailable() {
  ...
  byId("art-key-hex").innerHTML =
    copyChip(live[0].publicKey, live[0].publicKey) +
