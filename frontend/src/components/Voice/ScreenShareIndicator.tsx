...
export const ScreenShareIndicator: React.FC<Props> = ({
  ...
  if (isLocal && !remote) {
    // Local-only: passive "LIVE" badge. No view button — a user
    // can't watch their own stream meaningfully. No paused state:
    // an idle capture (static screen) is indistinguishable to viewers
    // from a stream of unchanging frames, so flagging "paused" would
    // misrepresent a normal flow as a problem.
    return (
      <span
