...
(*                                                                         *)
(* SUITE GENERATIONS (#454 P4) -- WHY `Keys` STAYS A SINGLETON.  An        *)
(* earlier draft of this header claimed the PQ (conversation, generation,  *)
(* epoch) extension was "a CONFIG change (enlarge `Keys`), not a spec      *)
(* rewrite".  That was wrong, and acting on it would have been a hollow    *)
(* gesture.  Spec B has no action and no invariant that relates two keys:  *)
(* every variable is [Keys -> ...], every invariant is \A k \in Keys, and  *)
(* the only shared state is the global monotone `clock`.  A two-key config *)
(* is therefore a PRODUCT OF TWO INDEPENDENT COPIES -- sound, but it       *)
(* proves nothing the one-key config already proves, at >10 minutes of TLC *)
(* (measured) instead of seconds.                                          *)
(*                                                                         *)
(* The safety content of P4 is the RELATIONSHIP between lineages: a        *)
(* successor may open only at epoch 0, and only by closing the exact head  *)
(* epoch of its predecessor.  That needs actions, not constants, so it     *)
(* lives in Spec A (CommitLog.tla -- MigratePrepare / MigrateCommit and    *)
(* OpeningClosesTheHead), which was rewritten rather than reconfigured.    *)
(* Read `k` here as ONE LINEAGE, i.e. a (conversation, generation) pair:   *)
(* retention inside a lineage is untouched by P4, because a successor      *)
(* restarts at epoch 0 on a fresh chain.                                   *)
(*                                                                         *)
(* The one piece of Spec B a generation-aware retention floor WOULD need   *)
(* -- a (generation, since_epoch) floor, so a prune cannot strand a device *)
(* still catching up on a retired lineage -- belongs to #539's commit-log  *)
(* pruning and should be modelled against that code, not guessed at now.   *)
(***************************************************************************)
EXTENDS Naturals, Sequences, FiniteSets

CONSTANTS
    Keys,      \* abstract head/retention keys -- one LINEAGE, i.e. a
               \* (conversation, generation) pair (see the header)
