...
impl AppState {
    ...
    ///
    /// Legacy entry point — reads `db_key_{user_id}` from the OS keystore
    /// (creating it if absent). Pre-PIN load-bearing builds use this. The
    /// PIN-aware path is [`load_user_db_with_key`], driven by `set_pin` /
    /// `unlock` once `AppState.unlock` is populated.
    pub async fn load_user_db(&self, user_id: &str) -> crate::error::Result<()> {
        ...
        self.load_user_db_with_key(user_id, &db_key).await
    }
