...
What remains visible — and is **irreducible** for a store-and-forward server — is that conversations exist, roughly how many members each has, and when they are active; the social graph is still keyed by `user_id` in membership rows (per-conversation pseudonyms are the deferred v3, #489). Pollis does **not** claim anonymity, IP-hiding (the relay overlay, #455, is deferred), or post-quantum protection (#454, deferred).
...
- **Implementation:** `openmls` 0.8 (https://github.com/openmls/openmls), with a Pollis-defined `MlsStore` (`pollis-core/src/signal/mls_storage.rs`) implementing the `openmls_traits::storage::StorageProvider` trait against the local SQLCipher `mls_kv` table. The crypto/rand provider is **selected by ciphersuite** (`pollis-core/src/commands/mls/provider.rs`): all live traffic runs on `openmls_rust_crypto` 0.5 over the `RustCrypto` AEAD/HKDF/HPKE primitives, whose AES-GCM tag check is constant-time (`subtle`). `openmls_libcrux_crypto` 0.3 is present but reachable only from the post-quantum hybrid suite (#454), which no production path selects; it is not used for the classic suite because its AES-GCM decryption has an unpatched non-constant-time tag check (RUSTSEC-2026-0211). The routing invariant is pinned by `classic_provider_never_routes_to_libcrux` in `pollis-core/src/commands/mls/tests.rs`, which is also the stated basis of that advisory's `deny.toml` entry.
...
- **The MLS commit log** records every membership/key-change commit. Replaying it under its invariant proves no fork (no two commits share a `(conversation_id, epoch)`) and no epoch regression/replay. This closes the server's ability to fork a conversation, roll an epoch back, or equivocate between auditors (§1.1). Full detail in `docs/transparency.md`.
