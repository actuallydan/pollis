...
- ~~**Infra cutover**~~ — **done.** Both environments run a separate commit-log DB: `LOG_DB_URL` + `LOG_DB_ADMIN_TOKEN` are wired into `delivery-deploy-prod.yml` / `delivery-deploy-dev.yml`, which apply the `migrations-log/` series on every deploy, and clients carry the read-only `LOG_DB_TOKEN`. See `docs/goal-a-deploy-runbook.md` (marked complete) for the procedure as executed.
