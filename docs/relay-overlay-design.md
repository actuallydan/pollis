...
- **Slice 3 (measured 2026-08-11) — one-hop latency against the §6.4 budget.** Verdict: **the budget
  ...
  **The finding that matters is not the numbers, it is the selection policy.** `RelayPool::
  ...
  average only if the pool happens to be near the user; today it is a coin flip. **Fixed in #812:** `candidate_order()` now orders healthy endpoints by
  measured dial latency, rotating among those within 20 ms of the best (so load still spreads and a
  region's clients don't all pin to one node) and ignoring latency on every 20th dial (so a demoted
  endpoint can earn its way back). Fail-open is untouched — latency only reorders *healthy*
  endpoints. With that in, the US-East client above takes the ≈+21 ms path rather than coin-flipping
  onto ≈+123 ms.
...
3. **LiveKit signalling stays direct — decided in v1 (#813), not merely deferred.** It was scoped
   as a v1 item and then cut, and the reason is the plane split itself, so this is settled rather
   than pending:

   - **It would hide nothing.** Media (RTP) goes direct to our own LiveKit host by §6.4, so that
     host observes the client's address for every call regardless. Routing only the setup
     WebSocket through the overlay leaves the same operator learning the same IP from the same
     call, seconds later. The privacy gain is not small — it is zero.
   - **It would cost the off-path invariant.** `livekit-api` honours `HTTPS_PROXY` on the
     signalling WebSocket only, which is the right shape, but that branch is feature-gated behind
     `rustls-tls-native-roots`; we pin `rustls-tls-webpki-roots`. Enabling it widens LiveKit's
     TLS trust anchors in *every* mode including Off (`tokio-tungstenite`'s two additive `cfg`
     blocks add native and webpki roots to one store), breaking "Off is byte-for-byte the direct
     path" in the trust dimension. Without it the SDK emits the CONNECT and then hard-fails, with
     no internal direct retry — so voice would break in `prefer` too, since the overlay's
     prefer→direct fallback is SOCKS-level and never sees a failure this far up the stack.
   - **`HTTPS_PROXY` is process-wide**, so it would also capture three clients that are direct by
     design: the signed-directory/revocation fetch (which bootstraps the pool it cannot yet route
     through — in `strict` with an empty pool that cannot recover), Expo push registration (item 2
     above, outside the allowlist by policy), and the LiveKit warmup probe.

   Reopening this needs a change to the **plane split**, not a better transport: overlaying
   signalling is only worth anything if media is overlaid too, and §6.4 rules that out on latency.
   An upstream connector seam in the `livekit` crate would remove the second and third objections
   but not the first.
