...
| F4 | ~~**Fragile delivery accounting**~~ — **FIXED (#1087)** | delivery was tracked by `sent_at` vs a per-device `conversation_watermark` timestamp — clock skew, equal timestamps, coarse acks. Replaced by a DS-assigned monotone per-conversation `seq` + `last_seq` cursor; `sent_at` is display metadata and nothing routes on it. The counter lives in `conversation_seq`, NOT `MAX(seq)`, because envelope GC deletes rows and a reset counter reuses a position every cursor has passed — the #692 shape, reappearing inside the fix | the tombstone floor, the `datetime('now')` seed trap (#908) and the tie rule were all costs of a lexically-compared string written by two clocks; all three are gone. Pinned by `delivery_sequence_tests` (incl. the GC-reuse case, which fails on the naive `MAX(seq)+1`) |
...
- **Phase 2:** I3 — **done (#1087).** The 30-day TTL is deleted and envelope
  retention is bound to the slowest member-device (kills F3, and F6 for
  envelopes); the delivery cursor is now a monotonic per-conversation `seq` plus
  a per-(user, device) `last_seq`, replacing the `sent_at` timestamp watermark
  (kills F4). The one non-obvious part is where the counter lives: `MAX(seq)` over
  `message_envelope` is wrong because GC deletes rows, so the high-water mark sits
  in `conversation_seq`, which GC never touches and only teardown removes.
