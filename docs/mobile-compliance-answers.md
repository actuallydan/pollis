...
| Avatar | `users.avatar_url` | Optional — points to a **public, unencrypted** R2 object (profile decoration, not message content; anyone with the URL can fetch it). | `pollis-core/src/commands/r2.rs` "Public objects (avatars, group icons)" |
...
| Push token | `push_token` table | **Yes** — device push token, best-effort, for content-free notifications. Payload carries a fixed title/body plus `{ h }` — an opaque per-notification handle resolved over the app's own authenticated channel — never the conversation, a sender, or a preview. Retained until the device re-registers or the account is deleted; **no expiry sweep** for uninstalled apps. | migration `000006_push_token.sql`; `mobile/lib/push/`; retention policy §4 |
