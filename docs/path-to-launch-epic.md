...
| PL-19 | Add APNs/FCM credentials + EAS `projectId`. **`projectId` done (#707)** — until then push was not "awaiting verification" but *structurally dead*: with the field absent `getExpoPushTokenAsync` cannot mint a token, so registration returned `null` on every device. The DS also now authenticates its Expo sends with `EXPO_TOKEN`. **Still open:** the APNs `.p8` (interactive Apple Developer Portal login) and FCM v1 service account + committed `google-services.json` — both account actions, tracked on **#707**. Delivery to a locked physical device still rides with PL-24, which needs the store-signed build. | P1 | S |
