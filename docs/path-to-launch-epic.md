...
| PL-05b | ~~**#661**~~ — **closed.** Root cause was never reproduced; the issue was closed rather than left as a standing unknown. Original analysis retained below. Was: root cause UNCONFIRMED. An earlier hypothesis was checked and ruled out; see "PL-05 analysis (b)". Instrument before touching the assertion. | P2 | M |
...
| PL-18 | Stand up EAS build profiles + release signing. **Account-free half landed (#706):** `mobile/eas.json` authored (dev/preview/production, `fingerprint` runtimeVersion so a native-core change can't ship a bad JS-only OTA), and `mobile-core-check.yml` now builds a real Android APK end to end (uniffi gen → 3-ABI `cargo-ndk` → `expo prebuild` → Gradle release, `expo prebuild`+Gradle directly, **not** `eas build`), uploaded as an artifact + `expo-doctor` in CI. **EAS project landed (#707):** `@pollis/mobile`, `projectId` + `owner` in `app.json`; `runtimeVersion` moved out of the build profiles into the app config, where the schema actually allows it (until then EAS CLI rejected `eas.json` outright). **Blocked remainder:** iOS build + Apple distribution signing (Apple Developer account #723 + current-gen Mac / Xcode 26); Android Play **upload** signing (Play console — #344/PL-17). The APK is debug-keystore-signed only until upload signing exists. | P1 | M |
| PL-19 | Add APNs/FCM credentials + EAS `projectId`. **`projectId` done (#707)** — until then push was not "awaiting verification" but *structurally dead*: with the field absent `getExpoPushTokenAsync` cannot mint a token, so registration returned `null` on every device. The DS also now authenticates its Expo sends with `EXPO_TOKEN`. **Credentials done too (#707):** APNs key `DZB37YSPU7` generated on the Apple portal by `eas credentials` and held by EAS, and an FCM v1 service account for Firebase project `pollis-38b6f`, with `mobile/google-services.json` committed and wired as `expo.android.googleServicesFile`. Delivery to a locked physical device still rides with PL-24, which needs the store-signed build. | P1 | S |
...
| PL-22 | Residual parity gaps (**#623 itself is closed**; these are the leftovers): wire `dm/info` actions; handle `all_mention` / `member_role_changed` / `roster_changed` (decoded then ignored); `device_revoked` self-sign-out; verify iOS keystore on device. | P2 | M |
