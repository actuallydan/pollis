...
- **Push credentials (#707):** the EAS project exists, so devices can now mint push tokens — but Expo can only *deliver* once the per-platform credentials are uploaded to it, and both are operator actions rather than code. **iOS:** an APNs `.p8` key, generated by `eas credentials -p ios` (interactive: an Apple Developer Portal login with a 2FA code — the `APPLE_PASSWORD` in Doppler is an app-specific password, which the portal does not accept). **Android:** an FCM v1 service-account JSON from a Firebase project, uploaded via `eas credentials -p android`, **plus** a `google-services.json` committed under `mobile/` and referenced by `expo.android.googleServicesFile` — without it `getExpoPushTokenAsync` throws on Android, because Expo wraps an FCM device token it cannot obtain. Until both land, registration succeeds and delivery does not.
