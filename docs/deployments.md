...
- **Still manual / blocked (PL-18 remainder):** iOS build + Apple distribution signing (needs an Apple Developer account #723 + a current-gen Mac with Xcode 26); Android Play **upload signing** (needs the Play console). The `mobile/CLAUDE.md` "Environment" notes cover running the same build on a workstation.
