...
- **WHERE it runs is pinned, and must stay pinned (#658).** `worker/index.ts` resolves the container with an explicit `locationHint: "enam"` (eastern North America — co-located with the `aws-us-east-1` Turso primary both environments use) under the object name `DS_SINGLETON_NAME`. This is not cosmetic: a named Durable Object is placed in the region nearest **whoever first instantiated it**, and that placement is **permanent for that object ID**. The previous `getContainer(binding, name)` call passed no hint (it is exactly `idFromName` + `get`), so placement was decided by whichever request happened to arrive first — prod landed next to the database, **dev landed far from it, by luck alone**. Measured from one IAD edge, identical code and image: prod `/version` ~47 ms vs dev ~287 ms, and a signed endpoint (one Turso read) ~68 ms vs ~1 530 ms — while the dev database itself answered in ~20 ms queried directly. The database was never slow; the container was simply far from it. **The hint applies only at first creation.** It does not move either environment's existing container; it decides where one lands the next time an object is created from scratch. That is what removes the silent-relocation risk from prod.

  - **Re-placing an EXISTING container needs a two-step instance-cap migration — never a bare rename.** Bumping `DS_SINGLETON_NAME` creates a *second* durable object while `max_instances: 1` permits only one container instance, so the incoming object can never start. Tried on 2026-08-06 against dev; the deploy's verify step stalled and every request 500'd with `Failed to start container: Maximum number of running container instances exceeded` until traffic was routed back to the original name (~15 min of dev downtime; prod was not deployed and was unaffected). The DO is stateless — all DS state is in Turso — so the blocker is the cap, not data. The correct sequence, if a re-placement is ever actually wanted:
