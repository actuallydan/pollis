...
- **Library:** `openmls` 0.8, with a Pollis-defined `StorageProvider` backed by the local SQLCipher `mls_kv` table. The **crypto provider is routed by ciphersuite** (`pollis-core/src/commands/mls/provider.rs`): `PollisProvider` = `openmls_rust_crypto` 0.5 serves the classic AES-GCM suite and is the only provider any production path constructs; `PollisPqProvider` = `openmls_libcrux_crypto` 0.3 exists solely for the post-quantum hybrid suite of #454, which nothing selects yet. The split is not cosmetic — libcrux's AES-GCM decryption has an unpatched non-constant-time tag check (RUSTSEC-2026-0211), so classic traffic must not be routed through it.
- **Cipher suite:** `MLS_128_DHKEMX25519_AES128GCM_SHA256_Ed25519` (`CS_CLASSIC`, suite 1, MTI for MLS 1.0). HPKE per RFC 9180. The suite is an explicit argument to the two functions that create suite-bound material — `create_mls_group_in_suite` and `build_key_package_in_suite` — rather than a hidden constant, and every production caller passes `CS_CLASSIC`. `CS_HYBRID` (`MLS_256_XWING_CHACHA20POLY1305_SHA256_Ed25519`, code point `0x004D`) is compiled and tested but selected by nothing; the DS stores each key package's suite in `mls_key_package.ciphersuite` so a claim can be narrowed to one pool. Signing is Ed25519 under both suites (`SIGNATURE_SCHEME`), so a device has ONE signing key and one `device_cert` regardless of suite.
