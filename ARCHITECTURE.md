...
| Local DB | SQLite via `rusqlite` 0.37 with bundled SQLCipher; per-user file `pollis_{user_id}.db`. **Not on Windows** — SQLCipher's OpenSSL cannot link beside libwebrtc's BoringSSL under MSVC, so Windows gets plain SQLite and an unencrypted local file (whitepaper §7.0, #992) |
...
| **Local SQLite (SQLCipher; plain SQLite on Windows — #992)** | Decrypted message plaintext (`message.content`), MLS group state (`mls_kv`), preferences cache, UI state | User profiles, groups, channels (fetched from the DS) |
