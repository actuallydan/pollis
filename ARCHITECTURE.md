...
- **Library:** `openmls` 0.8 + `openmls_libcrux_crypto` 0.3 (the libcrux-backed crypto/rand provider), with a Pollis-defined `StorageProvider` backed by the local SQLCipher `mls_kv` table. libcrux (rather than `openmls_rust_crypto`) is used because it implements the post-quantum hybrid ciphersuite required by later #454 phases while serving the classic suite identically and wire-compatibly (proven by the cross-provider interop test in `pollis-core/src/commands/mls/tests.rs`).
