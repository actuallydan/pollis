...
- **Library:** `openmls` 0.8, with a Pollis-defined `StorageProvider` backed by the local SQLCipher `mls_kv` table. The **crypto provider is routed by ciphersuite** (`pollis-core/src/commands/mls/provider.rs`): `PollisProvider` = `openmls_rust_crypto` 0.5 serves the classic AES-GCM suite; `PollisPqProvider` = `openmls_libcrux_crypto` 0.3 serves the post-quantum hybrid suite. Providers are **routed, never swapped** — the split is security-load-bearing, because libcrux's AES-GCM decryption has an unpatched non-constant-time tag check (RUSTSEC-2026-0211), so classic traffic must not be routed through it; the hybrid suite's AEAD is ChaCha20-Poly1305, which the advisory does not touch. Pinned by the `classic_provider_never_routes_to_libcrux` test.
- **Cipher suites (two).** `CS_CLASSIC` = `MLS_128_DHKEMX25519_AES128GCM_SHA256_Ed25519` (suite 1, MTI for MLS 1.0). `CS_HYBRID` = `MLS_256_XWING_CHACHA20POLY1305_SHA256_Ed25519` (code point `0x004D`), whose KEM is X-Wing = X25519 + ML-KEM-768 (FIPS 203), so a recorded session stays sealed unless *both* are broken. HPKE per RFC 9180. The suite is an explicit argument to the two functions that create suite-bound material — `create_mls_group_in_suite` and `build_key_package_in_suite` — rather than a hidden constant. Devices publish KeyPackage pools in **both** suites (`replenish_key_packages` iterates both); the DS stores each key package's suite in `mls_key_package.ciphersuite` so a claim can be narrowed to one pool, and derives `user_device.pq_capable` from the arrival of a hybrid pool. Signing is Ed25519 under both suites (`SIGNATURE_SCHEME`), so a device has ONE signing key and one `device_cert` regardless of suite.
- **Which suite a group runs.** `suite_for_new_group` (`pollis-core/src/commands/mls/group_state.rs`) births a group hybrid only when *both* gates pass: every device on the desired roster is `pq_capable`, **and** the deployment-wide fleet is complete — no unrevoked `user_device` seen within `FLEET_DORMANCY_DAYS` (90) is still classic-only. The roster gate alone is nearly vacuous, since a group cannot see tomorrow's invitees. Either gate failing (or failing to evaluate) keeps the group classic — it fails toward availability.
- **Suite migration** (#454 P4) does not change a running group's suite, which RFC 9420 forbids. It stands up a **successor group** in `CS_HYBRID`, moves the roster by Welcome, and retires the predecessor. Ordering is the monotone key `(conversation_id, generation, epoch)`; local `GroupId` is the conversation id verbatim at generation 0 and `"{conversation_id}#g{generation}"` after, tracked in the `mls_generation` table. The DS accepts generation N+1 at epoch 0 only when its `closes_epoch` names the head of generation N (`pollis_delivery::commit::accepts()`, Kani-proved) — a compare-and-swap that makes two competing successors unrepresentable. Migration is forward-only: pre-crossing traffic stays classically sealed.
